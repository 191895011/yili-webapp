...
export default {
  ...
  methods: {
    httpRequest (options) {
      ...
      keepPostdata () {
				if (this.filedata.length > 0) {
                    // 如果图片是服务器返回的图片，怎么处理
                    let ProgressArr = this.$store.state.entry.ProgressArr //进展程度
                    ...
                    // 进度条
                    if (!AddBoolean) {
                       this.$store.commit('EntryProgressPush', ID)
                       console.log('新的文件')
                    } else {
                      console.log('重复了')
                    }
      ...
      FileDataToVuex (FILEIDIndex, data) {
         // 保存本地信息
          this.$store.commit('localImgPush', this.localhostImgData)                 
       
        if (FILEIDIndex || FILEIDIndex === 0) {
            console.log('重新替换附件')
             this.$store.commit('FILEDATAReplace', data, FILEIDIndex)
         } else {
            console.log('新上的文件')
            this.$store.commit('FILEDATAPush', data)
        }
      },
      getObjectURL (file) {
            var url = null ;
    ...
    mounted () {
        ...
        let localList = this.$store.state.entry.LocalImgList
        let uploadarr = []
        // 查找本地数据
