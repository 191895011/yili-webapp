...
<div>
   <div >
    ...
    <mt-popup
      ...
      <mt-picker :slots="schoolslots" @change="onValuesschoolChange" value-key="name" class="levelselect" showToolbar>
   ...
   <mt-popup
        ...
        position="bottom"
        popup-transition="popup-fade">
      <mt-picker :slots="educationslots" @change="onValueseducationChange" value-key="name" class="levelselect" showToolbar>
           <div class="picker-toolbar-title">
   ...
   <mt-popup
        ...
        position="bottom"
        popup-transition="popup-fade">
      <mt-picker :slots="Professionslots" @change="onValuesProfessionChange" value-key="name" class="levelselect" showToolbar>
           <div class="picker-toolbar-title">
   ...
   <mt-popup
        ...
        position="bottom"
        popup-transition="popup-fade">
      <mt-picker :slots="SecDegreeslots" @change="onValuesSecDegreeChange" value-key="name" class="levelselect" showToolbar>
         <div class="picker-toolbar-title">
...
export default {
  ...
  data () {
    return {
      ...
      TeachData: {
        ...
        UserType: this.$store.state.TYPEA
      },
      schoolslots: [
        {
          flex: 1,
          values: ValueList.SchoolType,
          textAlign: 'center'
        }
      ],
      educationslots: [
        {
          flex: 1,
          values: ValueList.EducationList,
          textAlign: 'center'
        }
      ],
      Professionslots: [
        {
          flex: 1,
          values: ValueList.ProfessionTypeList,
          textAlign: 'center'
        }
      ],
      TeachModusslots: [
        {
          flex: 1,
          values: ValueList.TeachTypeList,
          textAlign: 'center'
        }
      ],
      SecDegreeslots: [
  ...
  methods: {
    ...
    teachData_keep () {
      ...
      const TeachData = this.TeachData
      const ID = this.$route.query.name
      console.log('用户类型' + TeachData.UserType)
      ...
      // if B user type ProfessionType  not empty
      // if (!TeachData.UserType) {
      //   if (this.DegreeNumber_select) {
      //     if (TeachData.ProfessionType === '') {
      //       TeachData.ProfessionTypeAcitve = true
      //       msgSelected = false
      //     } else {
      ...
      if (msgSelected) {
        if (ID === undefined) {
          ...
          let addmsg = {
            ...
            educationcode_set: this.TeachData.educationcode_set,
            unitTypecode_set: this.TeachData.unitTypecode_set,
            TeachModuscode_set: this.TeachData.TeachModuscode_set,
            ProfessionTypecode_set: this.TeachData.ProfessionTypecode_set,
            SecDegreecode_set: this.TeachData.SecDegreecode_set
          }
          this.$store.commit('TeachbgAddMsg', addmsg)
          this.$router.push({
            path: `/member`
    ...
    User_SelectschoolSure () {
      console.log('学校类型是否为空')
      console.log(this.TeachData.unitType_set)
      if (this.TeachData.unitType_set === '' || this.TeachData.unitType_set === '211高等学校') {
        this.TeachData.unitType_set = this.schoolslots[0].values[0].name
        this.TeachData.unitTypecode_set = this.schoolslots[0].values[0].code
      }
      this.TeachData.unitType = this.TeachData.unitType_set
      this.schoolVisible = !this.schoolVisible
      console.log('学校类型' + this.TeachData.unitTypecode_set)
    },
    onValuesschoolChange (picker, values) {
      this.TeachData.unitType_set = values[0].name
      this.TeachData.unitTypecode_set = values[0].code
    },
    // education select
    ...
    User_SelecteducationSure () {
      // 学历选择
      if (this.TeachData.education_set === '' || this.TeachData.education_set === '初中及以下') {
        this.TeachData.education_set = this.educationslots[0].values[0].name
        this.TeachData.educationcode_set = this.educationslots[0].values[0].code
      }
      ...
      if (education === '大专' || education === '大学' || education === '硕士研究生' || education === '博士研究生' || education === '博士后') {
        ...
        this.TeachData.unitTypecode_set = 99
      }
      // Teach Modus
      if (education === '初中及以下' || education === '高中') {
        ...
        this.TeachData.TeachModuscode_set = 1
      } else {
    ...
    User_SelecteProfessionSure () {
      if (this.TeachData.ProfessionType_set === '' || this.TeachData.ProfessionType_set === '哲学') {
        this.TeachData.ProfessionType_set = this.Professionslots[0].values[0].name
        this.TeachData.ProfessionTypecode_set = this.Professionslots[0].values[0].code
      }
      this.TeachData.ProfessionType = this.TeachData.ProfessionType_set
      this.ProfessionVisible = !this.ProfessionVisible
      console.log('专业类别' + this.TeachData.ProfessionTypecode_set)
    },
    onValuesProfessionChange (picker, values) {
      this.TeachData.ProfessionType_set = values[0].name
      this.TeachData.ProfessionTypecode_set = values[0].code
    },
    // Teach Modus
    ...
    User_SelecteTeachModusSure () {
      if (this.TeachData.TeachModus_set === '' || this.TeachData.TeachModus_set === '普通全日制') {
        this.TeachData.TeachModus_set = this.TeachModusslots[0].values[0].name
        this.TeachData.TeachModuscode_set = this.TeachModusslots[0].values[0].code
      }
      this.TeachData.TeachModus = this.TeachData.TeachModus_set
      this.TeachModusVisible = !this.TeachModusVisible
      console.log('教育形式' + this.TeachData.TeachModuscode_set)
    },
    onValuesTeachModusChange (picker, values) {
      this.TeachData.TeachModus_set = values[0].name
      this.TeachData.TeachModuscode_set = values[0].code
    },
    ...
    User_SelecteSecDegreeSure () {
      if (this.TeachData.SecDegree_set === '' || this.TeachData.SecDegree_set === '是') {
        this.TeachData.SecDegree_set = ValueList.NoList[0].name
        this.TeachData.SecDegreecode_set = ValueList.NoList[0].code
      }
      this.TeachData.SecDegree = this.TeachData.SecDegree_set
      this.SecDegreeVisible = !this.SecDegreeVisible
      console.log('第二学位' + this.TeachData.SecDegreecode_set)
    },
    onValuesSecDegreeChange (picker, values) {
      this.TeachData.SecDegree_set = values[0].name
      this.TeachData.SecDegreecode_set = values[0].code
    },
    teachData_remove () {
      const ID = this.$route.query.name
      if (ID === undefined) {
  ...
  mounted () {
    const usertype = this.$store.state.TYPEA
    ...
    if (ID === undefined) {
      ...
      console.log('用户情况' + TeachData.UserType)
    } else {
      ...
      list.forEach(function (item) {
        if (item.id === ID) {
          ...
          TeachData.educationcode_set = item.educationcode_set
          TeachData.unitTypecode_set = item.unitTypecode_set
          TeachData.TeachModuscode_set = item.TeachModuscode_set
          TeachData.ProfessionTypecode_set = item.ProfessionTypecode_set
          TeachData.SecDegreecode_set = item.SecDegreecode_set
        }
      })
      let education = this.TeachData.education
